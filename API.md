...
* `options [object]`: Optional additional options to pass on to [`mqtt.connect()`](https://www.npmjs.com/package/mqtt#connect), for example to use TLS download [mqtt-ca.pem](http://preview.console.thethingsnetwork.org/mqtt-ca.pem) and do something like:
